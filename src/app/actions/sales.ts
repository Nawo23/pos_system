...
export async function checkout(items: CartItem[], discount: number, paymentMethod: 'CASH' | 'CARD' | 'BANK_TRANSFER', customerId?: string) {
    ...
    const sale = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
        const sale = await tx.sale.create({
            data: {
                invoiceNo, userId: user.id, customerId, subtotal, discount, total, paymentMethod,
                items: {
                    create: items.map((i) => ({
